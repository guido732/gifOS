	...
	switch (type) {
		case "window":
			...
			<img class="img-element loading-animation" src="${element.images.original.url}" alt="${element.title}" /> 	
				<a href="${element.bitly_url}" target="_blank" type="button" class="btn-primary2 tag"><span class="btn-text-container" >Ver más...</span></a>
			</div>
		</div>`;
		...
		<span>${element.title}</span>
			...
			return $container.firstChild;
		case "tag":
			$container.innerHTML = `<button type="button" class="btn-primary2 tag"><span class="btn-text-container">${element.title}</span></button>`;
			return $container.firstChild;
	}
}
